...
function setupVerseCompletion() {
    ...
    wordOptions.forEach(option => {
        option.addEventListener("click", function() {
            if (this.classList.contains("used")) return; // Evitar usar la misma palabra dos veces
            ...
            if (emptyBlank) {
                ...
                this.disabled = true; // Deshabilitar la opción de palabra seleccionada
                ...
                checkAllChallengesCompletion(level); // Verificar si todos los desafíos están completos
            }
        });
    });
...
function checkVerseCompletion() {
    ...
    return allFilled && allCorrecfunction setupQuestions(questions) {
    document.querySelectorAll(".answer-option").forEach(option => {
        option.addEventListener("click", function() {
            ...
            checkAllChallengesCompletion(level);
        });
    });
}

function checkQuestionsCompletion(questions) {
    let allCorrect = true;
    questions.forEach((question, index) => {
        const selectedOption = document.querySelector(`[data-question="${index}"].selected`);
        if (!selectedOption || parseInt(selectedOption.dataset.answer) !== question.correct) {
            allCorrect = false;
        }
    });
    return allCorrect;
}ll(`[data-question="${questionIndex}"]`).forEach(opt => {
                opt.classList.remove("selected", "correct", "incorrect");
            });
...
function checkAllChallengesCompletion(level) {
    let allChallengesComplete = true;

    // Verificar completado del versículo
    if (level.verseCompletion) {
        if (!checkVerseCompletion()) {
            allChallengesComplete = false;
        }
    }

    // Verificar completado de preguntas
    if (level.questions) {
        if (!checkQuestionsCompletion(level.questions)) {
            allChallengesComplete = false;
        }
    }

    // Verificar completado del mini-reto
    if (level.miniChallenge) {
        if (!checkMiniChallengeCompletion(level.miniChallenge)) {
            allChallengesComplete = false;
        }
    }

    // Habilitar/deshabilitar el botón de completar nivel
    const completeLevelBtn = document.getElementById("complete-level-btn");
    if (completeLevelBtn) {
        completeLevelBtn.disabled = !allChallengesComplete;
    }

    return allChallengesComplete;
}

function checkMiniChallengeCompletion(challenge) {
    switch (challenge.type) {
        case 'prayer_order':
            // Lógica para verificar el orden de la oración
            const dropZones = document.querySelectorAll(".drop-zone");
            let currentOrder = [];
            dropZones.forEach(zone => {
                const item = zone.querySelector(".prayer-item");
                if (item) {
                    currentOrder.push(parseInt(item.dataset.itemIndex));
                }
            });
            return currentOrder.length === challenge.correctOrder.length && 
                   JSON.stringify(currentOrder) === JSON.stringify(challenge.correctOrder);
        case 'stealth_navigation':
            // Por ahora, devolver true para permitir el avance
            return true;
        case 'memory_puzzle':
            // Por ahora, devolver true para permitir el avance
            return true;
...
function checkLevelCompletion() {
    ...
    return checkAllChallengesCompletion(currentLevelData);
}
...
function calculateLevelPoints() {
    ...
    // Puntos por completar versículo
    if (currentLevelData.verseCompletion && checkVerseCompletion()) {
        points += 50;
    }

    // Puntos por preguntas correctas
    if (currentLevelData.questions) {
    ...
    // Puntos por mini-reto
    if (currentLevelData.miniChallenge && checkMiniChallengeCompletion(currentLevelData.miniChallenge)) {
        points += 100;
    }
...
function setupMiniChallenge(challenge) {
    switch (challenge.type) {
        case 'prayer_order':
            ...
            break;
        case 'stealth_navigation':
            setupStealthNavigationChallenge(challenge);
            break;
        case 'memory_puzzle':
            setupMemoryPuzzleChallenge(challenge);
            break;
        case 'construction_tetris':
            setupConstructionTetrisChallenge(challenge);
            break;
        case 'tower_defense':
            setupTowerDefenseChallenge(challenge);
            break;
        default:
            console.log(`Mini-challenge type ${challenge.type} not implemented yet`);
    }
}
...
function setupPrayerOrderChallenge(challenge) {
    ...
    prayerItems.forEach(item => {
        item.addEventListener('dragstart', function(e) {
            e.dataTransfer.setData('text/plain', this.dataset.itemIndex);
            this.classList.add('dragging');
        });
        ...
        // También permitir hacer clic para mover
        item.addEventListener('click', function() {
            const emptyZone = document.querySelector('.drop-zone:empty');
            if (emptyZone) {
                emptyZone.appendChild(this);
                checkAllChallengesCompletion(levels[gameState.currentLevel]);
            }
        });
    });
    ...
    dropZones.forEach(zone => {
        ...
        // Permitir hacer clic en zonas para limpiarlas
        zone.addEventListener('click', function() {
            if (this.children.length > 0) {
                const item = this.children[0];
                document.getElementById('prayer-items').appendChild(item);
                checkAllChallengesCompletion(levels[gameState.currentLevel]);
            }
        });
    });
...
function setupStealthNavigationChallenge(challenge) {
    // Implementación simplificada
    const stealthGame = document.getElementById('stealth-game');
    const message = document.createElement('div');
    message.className = 'stealth-message';
    message.innerHTML = `
        <p>¡Misión completada!</p>
        <p>Has navegado exitosamente por la corte real.</p>
        <button onclick="checkAllChallengesCompletion(levels[gameState.currentLevel])">Continuar</button>
    `;
    stealthGame.appendChild(message);
}

function setupMemoryPuzzleChallenge(challenge) {
    // Implementación simplificada
    const memoryGame = document.getElementById('memory-test');
    const message = document.createElement('div');
    message.className = 'memory-message';
    message.innerHTML = `
        <p>¡Memoria excelente!</p>
        <p>Has memorizado correctamente todos los elementos.</p>
        <button onclick="checkAllChallengesCompletion(levels[gameState.currentLevel])">Continuar</button>
    `;
    memoryGame.appendChild(message);
}

function setupConstructionTetrisChallenge(challenge) {
    // Implementación simplificada
    const constructionGame = document.getElementById('construction-game');
    const message = document.createElement('div');
    message.className = 'construction-message';
    message.innerHTML = `
        <p>¡Construcción completada!</p>
        <p>Has reparado exitosamente la sección del muro.</p>
        <button onclick="checkAllChallengesCompletion(levels[gameState.currentLevel])">Continuar</button>
    `;
    constructionGame.appendChild(message);
}

function setupTowerDefenseChallenge(challenge) {
    // Implementación simplificada
    const towerDefenseGame = document.getElementById('tower-defense-game');
    const message = document.createElement('div');
    message.className = 'tower-defense-message';
    message.innerHTML = `
        <p>¡Defensa exitosa!</p>
        <p>Has repelido todos los ataques enemigos.</p>
        <button onclick="checkAllChallengesCompletion(levels[gameState.currentLevel])">Continuar</button>
    `;
    towerDefenseGame.appendChild(message);
}

// Funciones de efectos de sonido (definidas en ui.js pero referenciadas aquí)
function playSuccessSound() {
    if (typeof window.UIEffects !== 'undefined' && window.UIEffects.playSuccessSound) {
        window.UIEffects.playSuccessSound();
    }
}

function playErrorSound() {
    if (typeof window.UIEffects !== 'undefined' && window.UIEffects.playErrorSound) {
        window.UIEffects.playErrorSound();
    }
}
